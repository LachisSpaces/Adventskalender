...
M.REFRESH_SUCCESS = hash("playfab.user_data.refresh_success")

--- Will be triggered when userdata failed to refresh
M.REFRESH_FAILED = hash("playfab.user_data.refresh_failed")
...
function M.set(key, value)
	local response, error = clientapi.UpdateUserData({ Data = { [key] = value }}, function(response) end, function(error) end)
	if error then
		return false, error
	else
