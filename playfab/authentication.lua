...
local M = {	listeners = listener.create() }
...
function M.has_username_and_password()
	local auth = savetable.load("auth")
	--pprint(auth)
...
function M.login(username, password)
	local auth = savetable.load("auth") or {}
...
function M.createAccount(generatedId)
	local auth = savetable.load("auth") or {}
	local request = {
		TitleId = playfab.settings.titleId,
